...
export const nosBrand = {
  ...
  appName: "Nos OS",
  companyName: "株式会社Nosテック",
  tagline: "SYSTEMS FOR HUMAN WORK.",
};

export function BrandMark({ className }: { className?: string }) {
  return (
    <span className={cn("relative block overflow-hidden rounded-panel border border-border bg-white", className)}>
...
export function BrandLockup({ className }: { className?: string }) {
  return (
    <div className={cn("rounded-panel border border-border bg-white p-3 shadow-sm", className)}>
...
export function BrandText({ compact = false }: { compact?: boolean }) {
  return (
    <div className="min-w-0">
      ...
      <p className="truncate text-xs text-slate-500 dark:text-slate-400">{nosBrand.companyName}</p>
    </div>
  );
}
